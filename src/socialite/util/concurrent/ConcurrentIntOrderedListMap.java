...
/**
 * A memory-efficient concurrent map with primitive int keys.
 ...
 *  public boolean replace(int key, V old, V value)
 *  public V put(int key, V value)
 *  public V putAtomic(int key, V value)
 *  public V putAtomicIfAbsent(int key, V value)
 *  public IntObjectIterator<V> iterator()
 *  public IntObjectIterator<V> iteratorFrom(int low, boolean lowInclusive)
 *  public IntObjectIterator<V> iteratorTo(int high, boolean highInclusive)
 *  public IntObjectIterator<V> iterator(int low, int high, boolean lowInclusive, boolean highInclusive)
 *  public class IntObjectIterator<V> {
 ...
 */
public class ConcurrentIntOrderedListMap<V> {
    /*
     ...
     */
    static ConcurrentIntSkipListMap.CallbackOnFound<Node> refCntCallback =
                    new ConcurrentIntSkipListMap.CallbackOnFound<Node>() {
    ...
    final Node head;
    ConcurrentIntSkipListMap<Node> skipListMap;

    public ConcurrentIntOrderedListMap() {
        skipListMap = new ConcurrentIntSkipListMap<Node>();
        head = new HeadNode(0);
    }
    ...
    private boolean isInconsistentNextNode(Node f, int key) {
    ...
    public boolean remove(int key, V old) {
        if (old == null)
    ...
    V doRemove(int key, V old) {
        return doReplace(key, old, null);
    }
    V doReplace(int key, V old, V value) {
        for (;;) {
            Node b = skipListMap.findGrandPredecessor(key, refCntCallback);
            if (b == null) b = head;
            if (b != head && b.first() <= key && key <= b.last()) {
    ...
    V putAtomic(int key, V value, boolean onlyIfAbsent) {
        if (value == null)
            throw new NullPointerException();
    ...
    static class Node {
        ...
        static private void nodeCopy(int[] srcKeys, Object[] srcVals, int srcBegin,
                                     int[] targetKeys, Object[] targetVals, int targetBegin, int copyLen) {
        ...
        int findKeyIndex(int key) { return Arrays.binarySearch(keys, 0, len(), key); }
        Object get(int key) {
        ...
        Object replace(int key, Object expect, Object value) {
            synchronized(this) {
        ...
        Object put(int key, Object value, ConcurrentIntOrderedListMap orderedMap) {
            int len = len();
            int pos = Arrays.binarySearch(keys, 0, len, key);
            if (pos >= 0) {
                Object old = vals[pos];
        ...
        // no concurrent read/write access is assumed
        private void putReally(int pos, int key, Object value, ConcurrentIntOrderedListMap orderedMap) {
            int len = len();
        ...
        boolean appendNewAtomic(int key, Object value, ConcurrentIntOrderedListMap orderedMap) {
            synchronized(this) {
        ...
        // concurrent read/write access is allowed
        Object putAtomic(int key, Object value, Node b, boolean onlyIfAbsent, ConcurrentIntOrderedListMap orderedMap) {
            synchronized(b) {
        ...
        private void putAtomicReally(Node b, int pos, int key, Object value, ConcurrentIntOrderedListMap orderedMap) {
            int len = len();
    ...
    public static class IntObjectIterator<V> {
        Node current;
        int len;
        int pos;
        int low, high;
        boolean lowInclusive, highInclusive;
        ConcurrentIntOrderedListMap orderedMap;
        int key;
        V val;
        IntObjectIterator(ConcurrentIntOrderedListMap map, int lo, int hi, boolean loIncl, boolean hiIncl) {
            low = lo;
            high = hi;
            orderedMap = map;
            lowInclusive = loIncl;
        ...
        int findBeginIndex(Node n, int key, boolean inclusive) {
        ...
        int findEndIndex(Node n, int key, boolean inclusive) {
            boolean exclusive = !inclusive;
    ...
    public IntObjectIterator<V> iterator() {
        return iterator(Integer.MIN_VALUE, Integer.MAX_VALUE, true, true);
    }
    public IntObjectIterator<V> iteratorFrom(int low, boolean lowInclusive) {
        return iterator(low, Integer.MAX_VALUE, lowInclusive, true);
    }
    public IntObjectIterator<V> iteratorTo(int high, boolean highInclusive) {
        return iterator(Integer.MIN_VALUE, high, true, highInclusive);
    }
    public IntObjectIterator<V> iterator(int low, int high, boolean lowInclusive, boolean highInclusive) {
        return new IntObjectIterator<V>(this, low, high, lowInclusive, highInclusive);
    }
    ...
    static void iterateFromTest() {
        final ConcurrentIntOrderedListMap<String> map1 = new ConcurrentIntOrderedListMap<String>();
        map1.put(25, "25");
        map1.put(30, "30");
        map1.put(31, "31");
        IntObjectIterator<String> it = map1.iteratorFrom(21, false);
        int sum=0;
        while (it.hasNext()) {
            it.next();
            sum += it.key();
    ...
    static void iterTest() {
        ...
        final ConcurrentIntOrderedListMap<String> map = new ConcurrentIntOrderedListMap<String>();
        for (int i=-1000; i<1001; i++) {
            map.put(i, ""+i);
        }

        IntObjectIterator<String> iter = map.iterator(11, 20, true, true);
        int sum = 0;
        while (iter.hasNext()) {
            iter.next();
            sum += iter.key();
