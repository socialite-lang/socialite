...
/**
 * A memory-efficient concurrent map with primitive long keys.
 ...
 *  public boolean replace(long key, V old, V value)
 *  public V put(long key, V value)
 *  public V putAtomic(long key, V value)
 *  public V putAtomicIfAbsent(long key, V value)
 *  public LongObjectIterator<V> iterator()
 *  public LongObjectIterator<V> iteratorFrom(long low, boolean lowInclusive)
 *  public LongObjectIterator<V> iteratorTo(long high, boolean highInclusive)
 *  public LongObjectIterator<V> iterator(long low, long high, boolean lowInclusive, boolean highInclusive)
 *  public class LongObjectIterator<V> {
 ...
 */
public class ConcurrentLongOrderedListMap<V> {
    /*
     ...
     */
    static ConcurrentLongSkipListMap.CallbackOnFound<Node> refCntCallback =
                    new ConcurrentLongSkipListMap.CallbackOnFound<Node>() {
    ...
    final Node head;
    ConcurrentLongSkipListMap<Node> skipListMap;

    public ConcurrentLongOrderedListMap() {
        skipListMap = new ConcurrentLongSkipListMap<Node>();
        head = new HeadNode(0);
    }

    public boolean contains(long key) { return get(key) != null; }
    ...
    private boolean isInconsistentNextNode(Node f, long key) {
    ...
    public boolean remove(long key, V old) {
        if (old == null)
    ...
    V doRemove(long key, V old) {
        return doReplace(key, old, null);
    }
    V doReplace(long key, V old, V value) {
        for (;;) {
            Node b = skipListMap.findGrandPredecessor(key, refCntCallback);
            if (b == null) b = head;
            if (b != head && b.first() <= key && key <= b.last()) {
    ...
    V putAtomic(long key, V value, boolean onlyIfAbsent) {
        if (value == null)
            throw new NullPointerException();
    ...
    static class Node {
        ...
        static private void nodeCopy(long[] srcKeys, Object[] srcVals, int srcBegin,
                                     long[] targetKeys, Object[] targetVals, int targetBegin, int copyLen) {
        ...
        int findKeyIndex(long key) { return Arrays.binarySearch(keys, 0, len(), key); }
        Object get(long key) {
        ...
        Object replace(long key, Object expect, Object value) {
            synchronized(this) {
        ...
        Object put(long key, Object value, ConcurrentLongOrderedListMap orderedMap) {
            int len = len();
            int pos = Arrays.binarySearch(keys, 0, len, key);
            if (pos >= 0) {
                Object old = vals[pos];
        ...
        // no concurrent read/write access is assumed
        private void putReally(int pos, long key, Object value, ConcurrentLongOrderedListMap orderedMap) {
            int len = len();
        ...
        boolean appendNewAtomic(long key, Object value, ConcurrentLongOrderedListMap orderedMap) {
            synchronized(this) {
        ...
        // concurrent read/write access is allowed
        Object putAtomic(long key, Object value, Node b, boolean onlyIfAbsent, ConcurrentLongOrderedListMap orderedMap) {
            synchronized(b) {
        ...
        private void putAtomicReally(Node b, int pos, long key, Object value, ConcurrentLongOrderedListMap orderedMap) {
            int len = len();
    ...
    public static class LongObjectIterator<V> {
        Node current;
        int len;
        int pos;
        long low, high;
        boolean lowInclusive, highInclusive;
        ConcurrentLongOrderedListMap orderedMap;
        long key;
        V val;
        LongObjectIterator(ConcurrentLongOrderedListMap map, long lo, long hi, boolean loIncl, boolean hiIncl) {
            low = lo;
            high = hi;
            orderedMap = map;
            lowInclusive = loIncl;
        ...
        int findBeginIndex(Node n, long key, boolean inclusive) {
        ...
        int findEndIndex(Node n, long key, boolean inclusive) {
            boolean exclusive = !inclusive;
    ...
    public LongObjectIterator<V> iterator() {
        return iterator(Long.MIN_VALUE, Long.MAX_VALUE, true, true);
    }
    public LongObjectIterator<V> iteratorFrom(long low, boolean lowInclusive) {
        return iterator(low, Long.MAX_VALUE, lowInclusive, true);
    }
    public LongObjectIterator<V> iteratorTo(long high, boolean highInclusive) {
        return iterator(Long.MIN_VALUE, high, true, highInclusive);
    }
    public LongObjectIterator<V> iterator(long low, long high, boolean lowInclusive, boolean highInclusive) {
        return new LongObjectIterator<V>(this, low, high, lowInclusive, highInclusive);
    }
    ...
    static void iterateFromTest() {
        final ConcurrentLongOrderedListMap<String> map1 = new ConcurrentLongOrderedListMap<String>();
        map1.put(25, "25");
        map1.put(30, "30");
        map1.put(31, "31");
        LongObjectIterator<String> it = map1.iteratorFrom(21, false);
        long sum=0;
        while (it.hasNext()) {
            it.next();
            sum += it.key();
    ...
    static void iterTest() {
        ...
        final ConcurrentLongOrderedListMap<String> map = new ConcurrentLongOrderedListMap<String>();
        for (int i=-1000; i<1001; i++) {
            map.put(i, ""+i);
        }

        LongObjectIterator<String> iter = map.iterator(11, 20, true, true);
        long sum = 0;
        while (iter.hasNext()) {
            iter.next();
            sum += iter.key();
